        ...
        <tbody class="text-gray-600 text-sm font-light">
            @forelse ($lampirans as $lampiran)
                <tr class="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td class="px-4 py-2">
                        <span
                            class="px-2 py-1 font-semibold leading-tight text-{{ $lampiran->status == 6 ? 'green' : 'orange' }}-700 bg-{{ $lampiran->status == 4 ? 'green' : 'orange' }}-100 rounded-full dark:text-white dark:bg-{{ $lampiran->status == 6 ? 'green' : 'orange' }}-600">
