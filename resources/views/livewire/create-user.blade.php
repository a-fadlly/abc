<div>
    <form class="p-10 bg-white rounded shadow-xl" wire:submit.prevent="saveUser">
        ...
        <div class="mt-2">
            ...
            <input value="{{ old('name') }}" class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded" id="name" name="name" type="text" placeholder="Name" aria-label="Name" wire:model.lazy="name">
        </div>
        @error('name')
        ...
        <div class="mt-2">
            ...
            <input value="{{ old('username') }}" class="w-full px-5 py-1 text-gray-700 bg-gray-200 rounded"
                id="username" name="username" type="text" placeholder="Username" aria-label="Username" wire:model.lazy="username">
        </div>
        @error('username')
            <div class="text-xs w-100 text-red-500 italic mt-2">{{ $message }}</div>
        @enderror
        <div class="mt-2">
            ...
            <input value="{{ old('email') }}" class="w-full px-5  py-1 text-gray-700 bg-gray-200 rounded"
                id="email" name="email" type="text" placeholder="Your Email" aria-label="Email" wire:model.lazy="email">
        </div>
        @error('email')
            <div class="text-xs w-100 text-red-500 italic mt-2">{{ $message }}</div>
        @enderror
        ...
        <div class="mt-2">
            ...
            <select value="{{ old('reporting_manager') }}" class="w-full px-5 py-3 text-gray-700 bg-gray-200 rounded"
                id="reporting_manager" name="reporting_manager" type="text" wire:model="reporting_manager">
                <option value="">-- Select role --</option>
